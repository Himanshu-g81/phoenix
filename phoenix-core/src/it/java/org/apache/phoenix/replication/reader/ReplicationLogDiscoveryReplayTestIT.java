...
public class ReplicationLogDiscoveryReplayTestIT extends BaseTest {
  ...
  public void testGetInProgressDirectoryProcessProbability() throws IOException {
    ...
    assertEquals("Should return custom value when config is set", 10.5, customResult, 0.001);
  }
  ...
  /**
   * Tests initialization in DEGRADED state with both in-progress and new files present. Validates
   * that lastRoundProcessed uses minimum timestamp and lastRoundInSync is preserved.
   */
  @Test
  public void testInitializeLastRoundProcessed_DegradedStateWithInProgressAndNewFiles()
    throws IOException {
    long currentTime = 1704153600000L; // 2024-01-02 00:00:00
    long inProgressFileTimestamp = 1704153420000L; // Earlier timestamp (00:57:00) - 3 min before
                                                   // current
    ...
    long expectedEndTime =
      (inProgressFileTimestamp / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1);
    ...
    testInitializeLastRoundProcessedHelper(currentTime, lastSyncStateTime, newFileTimestamp,
      inProgressFileTimestamp, HAGroupStoreRecord.HAGroupState.DEGRADED_STANDBY,
      expectedLastRoundProcessed, expectedLastRoundInSync,
      ReplicationLogDiscoveryReplay.ReplicationReplayState.DEGRADED, false);
  }

  @Test
  public void testInitializeLastRoundProcessed_DegradedStateWithBothFilesNewFileIsMin()
    throws IOException {
    long currentTime = 1704153600000L; // 2024-01-02 00:00:00
    long newFileTimestamp = 1704153420000L; // Earlier timestamp (00:57:00) - 3 min before current
    long inProgressFileTimestamp = 1704153540000L; // Later timestamp (00:59:00) - 1 min before
                                                   // current
  ...
  @Test
  public void testInitializeLastRoundProcessed_DegradedStateWithLastSyncStateAsMin()
    throws IOException {
    ...
    testInitializeLastRoundProcessedHelper(currentTime, lastSyncStateTime, newFileTimestamp, null, // no
                                                                                                   // in-progress
                                                                                                   // file
  ...
  public void testInitializeLastRoundProcessed_DegradedStateWithNoFiles() throws IOException {
    ...
    long expectedEndTime =
      (currentTime / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1);
    ReplicationRound expectedLastRoundProcessed =
      new ReplicationRound(expectedEndTime - roundTimeMills, expectedEndTime);
    ...
    testInitializeLastRoundProcessedHelper(currentTime, lastSyncStateTime, null, // no new file
  ...
  public void testInitializeLastRoundProcessed_SyncStateWithInProgressFiles() throws IOException {
    ...
    long expectedEndTime =
      (inProgressTimestamp / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1);
    ReplicationRound expectedLastRoundProcessed =
      new ReplicationRound(expectedEndTime - roundTimeMills, expectedEndTime);
    ...
    testInitializeLastRoundProcessedHelper(currentTime, null, // SYNC state - lastSyncStateTime not
                                                              // used
      null, // no new file
      inProgressTimestamp, HAGroupStoreRecord.HAGroupState.STANDBY, expectedLastRoundProcessed,
      expectedLastRoundInSync, ReplicationLogDiscoveryReplay.ReplicationReplayState.SYNC, false);
  }
  ...
  public void testInitializeLastRoundProcessed_SyncStateWithNewFiles() throws IOException {
    ...
    testInitializeLastRoundProcessedHelper(currentTime, null, // SYNC state - lastSyncStateTime not
                                                              // used
      newFileTimestamp, null, // no in-progress file
      HAGroupStoreRecord.HAGroupState.STANDBY, expectedLastRoundProcessed, expectedLastRoundInSync,
      ReplicationLogDiscoveryReplay.ReplicationReplayState.SYNC, false);
  }
  ...
  public void testInitializeLastRoundProcessed_SyncStateWithBothFiles() throws IOException {
    ...
    testInitializeLastRoundProcessedHelper(currentTime, null, // SYNC state - lastSyncStateTime not
                                                              // used
      newFileTimestamp, inProgressTimestamp, HAGroupStoreRecord.HAGroupState.STANDBY,
      expectedLastRoundProcessed, expectedLastRoundInSync,
      ReplicationLogDiscoveryReplay.ReplicationReplayState.SYNC, false);
  }
  ...
  public void testInitializeLastRoundProcessed_StandbyToActiveState() throws IOException {
    ...
    ReplicationRound expectedLastRoundProcessed =
      new ReplicationRound(currentTime - roundTimeMills, currentTime);
    ReplicationRound expectedLastRoundInSync =
      new ReplicationRound(currentTime - roundTimeMills, currentTime);
   ...
   * @param lastSyncStateTime          Last sync state time for HAGroupStoreRecord (use null for
   *                                   SYNC state)
   * @param newFileTimestamp           Timestamp for new file (use null to skip creating new file)
   * @param inProgressFileTimestamp    Timestamp for in-progress file (use null to skip creating
   *                                   in-progress file)
   ...
   */
  private void testInitializeLastRoundProcessedHelper(long currentTime, Long lastSyncStateTime,
    Long newFileTimestamp, Long inProgressFileTimestamp,
    HAGroupStoreRecord.HAGroupState haGroupState, ReplicationRound expectedLastRoundProcessed,
    ReplicationRound expectedLastRoundInSync,
    ...
    try {
      ...
      try {
        ...
        assertEquals("Last round processed should match expected", expectedLastRoundProcessed,
          lastRoundProcessed);
        ...
        assertEquals("Last round in sync should match expected", expectedLastRoundInSync,
          lastRoundInSync);

        // Verify state
        assertEquals("Replication replay state should match expected", expectedReplayState,
          discovery.getReplicationReplayState());

        // Verify failoverPending
        assertEquals("Failover pending should match expected", expectedFailoverPending,
  ...
  /**
   * Tests replay in SYNC state processing multiple rounds. Validates that both lastRoundProcessed
   * and lastRoundInSync advance together.
   */
  @Test
  public void testReplay_SyncState_ProcessMultipleRounds() throws IOException {
    ...
    try {
      ...
      try {
        ...
        ReplicationRound expectedRound1 =
          new ReplicationRound(initialEndTime, initialEndTime + roundTimeMills);
        assertEquals("First round should match expected", expectedRound1, processedRounds.get(0));

        ReplicationRound expectedRound2 = new ReplicationRound(initialEndTime + roundTimeMills,
          initialEndTime + (2 * roundTimeMills));
        assertEquals("Second round should match expected", expectedRound2, processedRounds.get(1));

        ReplicationRound expectedRound3 = new ReplicationRound(
          initialEndTime + (2 * roundTimeMills), initialEndTime + (3 * roundTimeMills));
        ...
        ReplicationRound lastRoundProcessed = discovery.getLastRoundProcessed();
        assertNotNull("Last round processed should not be null", lastRoundProcessed);
        assertEquals("Last round processed should be updated to 3rd round", expectedRound3,
          lastRoundProcessed);
        ...
        assertNotNull("Last round in sync should not be null", lastRoundInSync);
        assertEquals(
          "Last round in sync should be updated to match last round processed in SYNC state",
  ...
  /**
   * Tests replay in SYNCED_RECOVERY state with rewind to lastRoundInSync. Validates that processing
   * rewinds and re-processes from lastRoundInSync.
   */
  @Test
    ...
    try {
      ...
      try {
        ...
        ReplicationRound lastInSyncRound =
          new ReplicationRound(initialEndTime - roundTimeMills, initialEndTime);
        ReplicationRound currentRound = new ReplicationRound(initialEndTime + (2 * roundTimeMills),
          initialEndTime + (3 * roundTimeMills));
        ...
        // Verify the first round - starts from lastRoundInSync.endTime = initialEndTime (via
        // getFirstRoundToProcess)
        List<ReplicationRound> processedRounds = discovery.getProcessedRounds();
        ReplicationRound expectedFirstRound =
          new ReplicationRound(initialEndTime, initialEndTime + roundTimeMills);
        ...
        // After SYNCED_RECOVERY rewind, processing restarts from lastRoundInSync.endTime =
        // initialEndTime
        // This means round 1 is processed again, then rounds 2-5
        ReplicationRound expectedSecondRound =
          new ReplicationRound(initialEndTime, initialEndTime + roundTimeMills);
        assertEquals("Second round (first in SYNC, re-processing) should match expected",
          expectedSecondRound, processedRounds.get(1));

        ReplicationRound expectedThirdRound = new ReplicationRound(initialEndTime + roundTimeMills,
          initialEndTime + (2 * roundTimeMills));
        assertEquals("Third round should match expected", expectedThirdRound,
          processedRounds.get(2));

        ReplicationRound expectedFourthRound = new ReplicationRound(
          initialEndTime + (2 * roundTimeMills), initialEndTime + (3 * roundTimeMills));
        assertEquals("Fourth round should match expected", expectedFourthRound,
          processedRounds.get(3));

        ReplicationRound expectedFifthRound = new ReplicationRound(
          initialEndTime + (3 * roundTimeMills), initialEndTime + (4 * roundTimeMills));
        assertEquals("Fifth round should match expected", expectedFifthRound,
          processedRounds.get(4));

        ReplicationRound expectedSixthRound = new ReplicationRound(
          initialEndTime + (4 * roundTimeMills), initialEndTime + (5 * roundTimeMills));
        assertEquals("Sixth round should match expected", expectedSixthRound,
          processedRounds.get(5));

        // Verify lastRoundProcessed was updated
        ReplicationRound lastRoundProcessed = discovery.getLastRoundProcessed();
        assertNotNull("Last round processed should not be null", lastRoundProcessed);
        assertEquals("Last round processed should be updated to 5th round", expectedSixthRound,
          lastRoundProcessed);
        ...
        assertNotNull("Last round in sync should not be null", lastRoundInSync);
        assertEquals(
          "Last round in sync should be updated to match last round processed after SYNC transition",
  ...
  /**
   * Tests state transition from SYNC to DEGRADED during replay processing. Validates that
   * lastRoundInSync is preserved at the last SYNC round.
   */
  @Test
  public void testReplay_StateTransition_SyncToDegradedDuringProcessing() throws IOException {
    ...
    try {
      ...
      try {
        ...
        ReplicationRound expectedRound4 = new ReplicationRound(
          initialEndTime + (3 * roundTimeMills), initialEndTime + (4 * roundTimeMills));
        assertEquals("Fourth round should match expected", expectedRound4, processedRounds.get(3));

        ReplicationRound expectedRound5 = new ReplicationRound(
          initialEndTime + (4 * roundTimeMills), initialEndTime + (5 * roundTimeMills));
        ...
        ReplicationRound lastRoundAfterReplay = discovery.getLastRoundProcessed();
        assertEquals("Last round processed should be updated to 5th round", expectedRound5,
          lastRoundAfterReplay);

        // Verify lastRoundInSync was updated only for first round (SYNC), then preserved
        // State changed to DEGRADED AFTER processing round 2, but BEFORE updating lastRoundInSync
        // for round 2
        // So lastRoundInSync should remain at round 1 (the last round fully completed in SYNC
        // state)
        ReplicationRound lastRoundInSync = discovery.getLastRoundInSync();
        assertNotNull("Last round in sync should not be null", lastRoundInSync);
        assertEquals(
          "Last round in sync should be preserved at round 1 (last fully completed SYNC round)",
  ...
  /**
   * Tests state transition from DEGRADED to SYNCED_RECOVERY and then to SYNC. Validates rewind
   * behavior and lastRoundInSync update after SYNC transition.
   */
  @Test
  public void testReplay_StateTransition_DegradedToSyncedRecovery() throws IOException {
    ...
    try {
      ...
      try {
        ...
        // Total: 2 + 5 = 7 calls
        // First call uses getFirstRoundToProcess() which starts from lastRoundInSync.endTime =
        // initialEndTime
        // After state change to SYNCED_RECOVERY, it rewinds and continues from
        ...
        ReplicationRound expectedRound1 =
          new ReplicationRound(initialEndTime, initialEndTime + roundTimeMills);
        assertEquals("First round should start from lastRoundInSync.endTime", expectedRound1,
          processedRounds.get(0));
        ...
        // Processing restarts from lastRoundInSync.endTime = initialEndTime (re-processing rounds
        // 1-2 and continuing)
        ReplicationRound expectedRound3 =
          new ReplicationRound(initialEndTime, initialEndTime + roundTimeMills);
        assertEquals("Third round (after rewind) should restart from lastRoundInSync.endTime",
          expectedRound3, processedRounds.get(2));

        ReplicationRound expectedRound4 = new ReplicationRound(initialEndTime + roundTimeMills,
          initialEndTime + (2 * roundTimeMills));
        assertEquals("Fourth round should match expected", expectedRound4, processedRounds.get(3));
        ...
        ReplicationRound expectedRound6 = new ReplicationRound(
          initialEndTime + (3 * roundTimeMills), initialEndTime + (4 * roundTimeMills));
        assertEquals("Sixth round should match expected", expectedRound6, processedRounds.get(5));

        ReplicationRound expectedRound7 = new ReplicationRound(
          initialEndTime + (4 * roundTimeMills), initialEndTime + (5 * roundTimeMills));
        ...
        ReplicationRound lastRoundAfterReplay = discovery.getLastRoundProcessed();
        assertEquals("Last round processed should be updated to 7th round", expectedRound7,
          lastRoundAfterReplay);

        // Verify lastRoundInSync was preserved during DEGRADED, then updated during SYNC
        // After transition to SYNC (from SYNCED_RECOVERY), lastRoundInSync should match
        // lastRoundProcessed
        ReplicationRound lastRoundInSync = discovery.getLastRoundInSync();
        assertNotNull("Last round in sync should not be null", lastRoundInSync);
        assertEquals(
   ...
   * Validates lastRoundInSync preservation during DEGRADED, rewind in SYNCED_RECOVERY, and update
   * in SYNC.
   */
  @Test
  public void testReplay_StateTransition_SyncToDegradedAndBackToSync() throws IOException {
    ...
    try {
      ...
      try {
        ...
        // - 8 more rounds in SYNC (rounds 7-14, from initialEndTime to initialEndTime +
        // 8*roundTimeMills)
        ...
        for (int i = 0; i < 2; i++) {
          ...
          assertEquals("Round " + (i + 1) + " (SYNC) should match expected", expectedRound,
        ...
        for (int i = 2; i < 5; i++) {
          ...
          long endTime = initialEndTime + ((i + 1) * roundTimeMills);
          ReplicationRound expectedRound = new ReplicationRound(startTime, endTime);
          assertEquals("Round " + (i + 1) + " (DEGRADED) should match expected", expectedRound,
            processedRounds.get(i));
        }
        ...
        ReplicationRound expectedLastRound = new ReplicationRound(
          initialEndTime + (9 * roundTimeMills), initialEndTime + (10 * roundTimeMills));
        assertEquals("Last round processed should be the 14th round", expectedLastRound,
          lastRoundAfterReplay);
  ...
  /**
   * Tests replay when no rounds are ready to process. Validates that lastRoundProcessed and
   * lastRoundInSync remain unchanged.
   */
  @Test
  public void testReplay_NoRoundsToProcess() throws IOException {
    ...
    try {
      ...
      try {
        ...
        assertEquals("processRound should not be called when no rounds to process", 0,
          discovery.getProcessRoundCallCount());
  ...
  public void testReplay_TriggerFailoverAfterProcessing() throws IOException {
    ...
    try {
      ...
      try {
        ...
        assertEquals("triggerFailover should be called exactly once", 1,
          discovery.getTriggerFailoverCallCount());
  ...
  private TestableReplicationLogTracker createReplicationLogTracker(final Configuration config,
    final String haGroupName, final FileSystem fileSystem, final URI rootURI) throws IOException {
  ...
  /**
   * Testable implementation of ReplicationLogTracker for unit testing. Exposes protected methods to
   * allow test access.
   */
  ...
  /**
   * Testable implementation of ReplicationLogDiscoveryReplay for unit testing. Provides dependency
   * injection for HAGroupStoreRecord, tracks processed rounds, and supports simulating state
  ...
  private static class TestableReplicationLogDiscoveryReplay extends ReplicationLogDiscoveryReplay {
    ...
    public TestableReplicationLogDiscoveryReplay(
    ...
    protected void processRound(ReplicationRound replicationRound) throws IOException {
      ...
      roundsProcessed++;
      if (
        stateChangeAfterRounds > 0 && roundsProcessed == stateChangeAfterRounds
          && newStateAfterRounds != null
      ) {
        LOG.info("Rounds Processed: {}, newStateAfterRounds: {}", roundsProcessed,
          newStateAfterRounds);
      ...
      if (
        failoverPendingChangeAfterRounds > 0 && roundsProcessed == failoverPendingChangeAfterRounds
      ) {
        LOG.info("Rounds Processed: {}, setting failoverPending to: {}", roundsProcessed,
          failoverPendingValueAfterRounds);
