...
public class HighAvailabilityGroup {
  ...
  private HighAvailabilityGroup(HAGroupInfo info, Properties properties) {
    ...
    // Get the cluster role record cache frequency from properties and if properties is not set,
    // then get it from config
    this.clusterRoleRecordCacheFrequency =
      Long.parseLong(properties.getProperty(PHOENIX_HA_CRR_CACHE_FREQUENCY_MS_KEY,
        config.get(PHOENIX_HA_CRR_CACHE_FREQUENCY_MS_KEY,
          String.valueOf(PHOENIX_HA_CRR_CACHE_FREQUENCY_MS_DEFAULT))));
  }
  ...
  /**
   * Get an instance of {@link HAURLInfo} given the HA connecting URL (with "|") and client
   * properties. Here we do parsing of url and try to extract principal and other additional params
  ...
  /**
   * checks if the given url is appropriate for HA Connections, HA URL will only work for RPC
   * Registry so it won't accept MASTER or ZK protocols but if no protocol is specified, it will be
   * assumed to be RPC
  ...
  private static String checkUrl(String url) throws SQLException {
    if (url.startsWith(PhoenixRuntime.JDBC_PROTOCOL_RPC)) {
      ...
      throwMalFormedConnectionUrlException(String.format(
        "Protocol specified is MASTER, which "
          + "is not acceptable for HA Connections, only RPC Registry is mandated, for url %s",
        url));
  ...
  static Optional<String> getFallbackCluster(String url, Properties properties)
    ...
    if (!fallbackCluster.startsWith(PhoenixRuntime.JDBC_PROTOCOL_RPC)) {
      fallbackCluster =
        PhoenixRuntime.JDBC_PROTOCOL_RPC + JDBC_PROTOCOL_SEPARATOR + fallbackCluster;
    }
   ...
   * expected to be passed in correct format i.e. zk1\\:port1,zk2\\:port2,zk3\\:port3,
   ...
   *         example :-
   *         jdbc:phoenix+zk:zk1\\:port1,zk2\\:port2,zk3\\:port3,zk4\\:port4,zk5\\:port5::znode:
   *         principal:additionalParams or jdbc:phoenix+master:master1\\:port1,master2\\:port2,
   *         master3\\:port3,master4\\:port4,master5\\:port5:::principal:additionParams
   */
    ...
    if (haURLInfo != null) {
      if (ObjectUtils.anyNotNull(haURLInfo.getPrincipal(), haURLInfo.getAdditionalJDBCParams())) {
        if (extraSeparator) {
          ...
          sb.append(JDBC_PROTOCOL_SEPARATOR).append(JDBC_PROTOCOL_SEPARATOR);
        }
      ...
      if (ObjectUtils.anyNotNull(haURLInfo.getAdditionalJDBCParams())) {
        sb.append(JDBC_PROTOCOL_SEPARATOR).append(haURLInfo.getAdditionalJDBCParams());
      }
    }
    return sb.toString();
  }
   ...
   * @return jdbc HA url in proper format i.e.
   *         jdbc:phoenix+rpc:[url1|url2]:principal:additionalParams example :-
   *         jdbc:phoenix+rpc:[master1\\:port1,master2\\:port2,master3\\:port3,master4\\:port4,
   *         master5\\:port5|peer_master1\\:port1,peer_master2\\:port2,peer_master3\\:port3,
   *         peer_master4\\:port4,peer_master5\\:port5]:principal:additionParams
   */
   ...
   * <p>
   * This method will get the new ClusterRoleRecord from RegionServer Endpoints and apply it to the
   * HA group, and it will call to transition the Policy's connections according to the new record.
   * <p>
   * This method uses a read-write lock pattern to allow multiple concurrent callers to check if a
   * refresh is needed while ensuring only one thread performs the actual refresh operation at a
   * time. This improves throughput by avoiding blocking all callers during the quick check phase.
  ...
  public boolean refreshClusterRoleRecord(boolean forceRefresh) throws SQLException {
    ...
    try {
      ...
      String transitionTimeoutProp = properties.getProperty(PHOENIX_HA_TRANSITION_TIMEOUT_MS_KEY,
        config.get(PHOENIX_HA_TRANSITION_TIMEOUT_MS_KEY));
  ...
  /**
   * Check if we should refresh the RoleRecord based on cache frequency if the cacheAge is greater
   * than clusterRoleRecordCacheFrequency, then return true
   */
